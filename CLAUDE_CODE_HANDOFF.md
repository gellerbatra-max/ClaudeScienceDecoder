...
| `plaid/` | (2026-09-25) two plaid markers, the ZZ-PLAID matching table and the engine's `frommed.mra` of job 117: the plaid / stripe doubles of section 1; sections 9 / 23 / 24 = the matching rules (v4.25: five markers + the engine's `frommed.mra` of each job; v4.32: `ZZPQ-M` + `ZZ-PLAID-Y2` with Y offsets) |
| `deg45/` | (2026-09-25) a W-row table with the 45-degree flip codes 9-12, its marker nested by AccuNest and the MarkPlot DXF: a 45-degree placement = tilt float +-45.0 on top of the code; `frame_ambiguous` (v4.33: the collar's frame there is decided by the slot bit) |
| `engine/` | (2026-09-25) the nest engine's own input file `frommed.mra` of four AccuNest jobs (made from `twoply/ZZQ-W`, `twoply/ZZQ-A`, `flipcount/ZZR-S`, `deg45/ZZR-45`): the per-instance ANGLE / FLIP_FLAG = `retrieval_orientation`, the per-category flags = the Piece Options, the outline = piece + block rectangle; read by `accumark_engine.py`; v4.26 adds ZZPV-M / ZZPS-M (buffers of 0.0059 / 0.0118 in) for the buffer-rectangle and frame checks; v4.27 adds ZZN-B5 and `INTOMED_HEADERS.json` (the engine's output header of five jobs: marker length, fabric cost / weight); v4.30 adds `*.job_settings.txt` (the Nest Markers settings of nine jobs) and the `ZZROT-*` frommed files (override jobs); v4.33 adds `FRAME_TURNS.json` (133 pieces x sizes of five jobs: the engine's quarter turn against the slot word @+60 bit 0x0200) |
| `tilt/` | (2026-09-25) a ZZLL-1 copy with unequal cw / ccw tilt limits and the markers made from it: the marker keeps min(cw, ccw); the twelve Piece Options as the editor words them (S = no flip pins a slot's chirality; `demand[].mirror_binding`) |
| `twoply/` | (2026-09-25) one model laid in three flip configurations (Model Editor FLIPS edited) with a table of each spread, plus AutoMark / AccuNest placements: slot bits 0x0080 = X, 0x0100 = Y; the two-ply merge rule (`slot['flip']`, `preset_turn_deg`, MARKER_FORMAT_SPEC.md section 24) |
