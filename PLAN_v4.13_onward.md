...
| v4.40 | Is the arch from the corner notch? | no: my DXF's layer 3 (curve) point at a vertex; clean corner notches verify exactly (`notchnum/ZZCN6`); the stream holds a corner-curved edge straight | `notchnum/ZZCN6*` | branch `v4.40-corner-notch-clean` |
| v4.41 | Can V17 make a turned piece? | no: PDS normalizes rotation on save; a stored tall piece (Rotate + Realign Grain) and one with a cross line are not turned | none | branch `v4.41-pds-orientation` |
| v4.42 | Corner notches above 15, made in PDS | clamped to code 5 like edge notches (12 / 16 / 25 / 9 / 20: all 5, piece and stream); a piece-decoder bug on a first-point corner notch fixed | `notchnum/ZZCN7*` | branch `v4.42-first-point-notch` |
| v4.43 | Section 5 (the Annotation table copy) | decoded on the byte from my own Annotation editor tables: rows + 30 token codes; 225 of 225 markers close | `annotation/` | branch `v4.43-annotation-copy` |
...
| 7 | **DONE v4.43 for section 5** (annotation table copy on the byte); words @484..@604 stay open (v4.38). Earlier: **MOSTLY DONE v4.27** (byte map: unidentified non-zero bytes 933-1,182 -> 105-371 per marker; fabric weight / cost @596 / @600 identified; open: section 5, the doubles @404 / @438, a few words): Section 1 counters, trailer state words, section 5 (Annotation copy) | Byte-map coverage | Offline | offline |
| 8 | **DONE v4.42** (corner notches above 15 made in PDS: clamped to 5). Earlier: **MOSTLY DONE v4.34** (offline: corner notches read and in the nest spec, a corner type 9 seen unclamped [hint]; open: a live PDS corner notch above 15, how the cutter draws corner vs edge notches): Notch numbers above 5 on a corner point; whether the cutter draws different shapes for numbers that read the same code | Notch fidelity | Live in PDS plus a marker plot | GUI-heavy |
