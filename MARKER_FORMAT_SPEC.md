...
* **Made in V17 on purpose (v4.41, live, own PDS, scratch pieces `ZZV-*`): nothing turns.** (1) PDS **normalizes a rotated piece on save**: `Modify > Rotate` 90 (grain rotates with the piece), then Save As - with or without `Set Original Position` - stores the piece with its ORIGINAL outline and a horizontal grain (the only difference is the stored position: `Set Original Position` puts it at the origin). A vertical-grain piece therefore cannot be stored by PDS, which is what v4.32 found. (2) `Rotate` with **`Realign Grain / Grade Ref` ticked** rotates the outline and leaves the grain horizontal: that piece IS stored tall (7.874 x 11.811 in, grain 3.94 in horizontal). Its marker slot does **not** set the turn bit and its stream and home box are the tall box. (3) The same tall piece with a vertical internal line across the grain (the jacket sleeve's cross): not turned either. So V17-made tall pieces, with or without a cross line, are not turned; the turned pieces of the corpus (the sample jacket sleeve, the sample blouse collar) come from AccuMark-9-era piece objects (the blouse pieces are the small legacy files `accumark_pds.decode` cannot read) and I cannot make one.
...
* **Numbers above 5 - settled live (v4.42): a corner notch is clamped to code 5 exactly like an edge notch.** In my own PDS (scratch piece `notchnum/ZZCN7-A`, the 25-row notch table as `P-NOTCH`, `Create > Notch > Standard`, Type set with the keyboard) I put notches of the numbers **12, 16, 25 and 9 on the four corners and 20 on an edge** of a rectangle: the piece stores **code 5 at every one of the five points** (f1 = 0x0500 on the corners, 0x0501 on the edge) and the marker's stream (`notchnum/ZZCN7.GT_mark`) reads `turn 5` x 4 and `notch 5`, the record verifies (93.000 / 39.370). The number lives in the piece's line table only, as the last byte of the 45-byte tag-0x07 child (v4.15): here only the two notches placed on the piece's line ends kept one (25 at the bottom-right corner, 20 on the bottom edge); the numbers 12 / 16 / 9 of the other three corners are stored nowhere I can find on this graded rectangle [?].
  The raw corner types 9 / 10 seen on six records of one real style earlier were artifacts of streams that do not verify (of the 480 records with a corner type on this machine, the 390 that verify carry only 1, 2 and 5): **no verified record has a corner type above 5.** (The nest spec still passes a type above 5 through as its number; it never occurs.)
* **Live (v4.35, 2026-09-25): a corner notch made by the DCU is clamped like an edge notch.** A DXF of a 30 x 20 cm rectangle with notch points on two vertices and one on an edge (`notchnum/ZZCN_spec.json`; all three become number 6 under the scratch table) was imported, an Easy Order of the model processed to an unmade marker (`notchnum/ZZCN.GT_mark`, piece `ZZCN-RECT.GT_piece`): the piece stores **code 5** on both corner notches (`is_corner_notch`, f1 = 0x0500) and on the edge notch (f1 = 0x0501, number 6 in its child), and the stream has `turn 5`, `notch 5`, `turn 5`. (The raw 9 of a real style that this bullet once cited was an artifact of unverified streams: see the v4.42 bullet.) (the ZZCN pieces of this bullet also carry the curve-point artifact below; `ZZCN6` repeats the corner notches without it).
...
* **The record that did not verify was my DXF's fault (v4.36 - v4.40): a curve point at a vertex.** `notchnum/ZZCN2` (the same rectangle three times: no notch / one edge notch / two corner notches) verified for the first two and not for the corner-notch piece: head area 101.364 / perimeter 39.677 against the stream polygon's 93.000 / 39.370, stored box 8.966 in high against 7.874. AccuNest job 302 (`engine/ZZCN2.frommed.mra`) showed the engine's polygon of that piece has **12 points, an ARCHED top edge** 1.092 in above its chord (its `AM_AREA` = the head's 101,364) - in neither the marker's stream (4 points) nor the piece object (the same 5 perimeter points as the plain piece). **Cause, found by the clean control (`notchnum/ZZCN6`): my DXF generator wrote every notch as a layer 4 point AND a layer 3 point (a CURVE point); at a vertex the DCU turned that into a curved edge.** The same rectangle with the notch points on layer 4 only imports as a plain piece with two corner notches (f1 = 0x0500, code 5) and **verifies exactly**: head area 93.000 / perimeter 39.370, home box = outline box, stream `turn 5`, `turn 5`. The notch table is irrelevant (a fully defined 25-row table gave the same 101.364). The generator is fixed (`make_aama_dxf.py`: no layer 3 point, no repeated vertex for a notch on a vertex).
  What stays a real finding: **a piece whose edge is curved by a curve point at a corner is curved for the engine and for AccuMark's declared area / perimeter / box, but the marker's stream holds it straight** - so on such a record the stream understates the piece and the record fails verification (the nest spec says `no outline`, loudly). Real styles with corner notches verify (390 records). `ZZCN` / `ZZCN2` stay as the fixtures of that case, `ZZCN6` is the clean corner-notch fixture.
* **The record as first seen (ZZCN, v4.35):** the same artifact. Since v4.35 the guess `record_outline` returns for an unverified record is the split with the longest first contour (here the whole 5-point outline with its notches).
...
Across the 222 distinct markers of this machine the unidentified non-zero bytes of section 1 sit at the file offsets 484 (u16, 38 values), 488 (u16, 42 values: 94 on 50 markers, 7 on 24), 494 (u16, 3 - 8), 504 (`5` on all 222), 506 (`0` on 142, 20 - 40 on the rest), 522 (`3750` on 204, `1476` on 18), 530 (u16, 59 values), 588 (`0x01010101` on 204, `0` on the 18 oldest), 592 (`0x8018` / `0x8218` ..., 9 values) and 604 (0 or 1). A brute-force correlation of each word with sums over the records / slots of every prefix word, the stream lengths, the text lengths, the record / slot / piece / size counts and the name lengths (221 markers) finds **no rule**: the best is `@484 = the number of records` on 38 of 221 and `@506 = sum of prefix word 3` on 53 of 221. None of these words is needed by a nester (they do not feed the engine's job file); they stay identified as `unknown`. Section 5 (the Annotation table copy, up to 9.6 KB) was decoded in v4.43 - section 39.

## 39. Section 5 = the annotation table copy [V, v4.43, live]

Section 5 is the marker's copy of its **Annotation table** (the table the Annotation editor writes and the order names): the plot text templates for the marker and its pieces - not job data, but now read on the byte. Made with my own Annotation editor (`annotation/ZZANN-1.GT_annot`) and an Easy Order whose Annotation column names it (`annotation/ZZAN-M.GT_mark`).

* **Rows.** `<u16 name length><u16 flags><u16 token count><name><tokens>` (the table object holds the same rows behind 4 zero bytes and a `u16` row count; the oldest vintage pads each name to 20 bytes and puts flags / count after it). The first row's header sits in the 6 bytes before the directory offset (the usual lead); the copy ends 6 bytes before the next section with the table's zero trailer.
* **Tokens** = the editor's `Annotation Format` types, one code byte each, plus a value where the type takes one (`0x..` = code): `01 MK` marker name, `13 MS` marker description, `02 ON` order number, `03 OD` order description, `04 MD` model name, `1c PS` plaid / stripe, `0b /` new line, `14 MSQ` model / size / quantity, `15 L` length, `16 U` utilization, `17 WI` marker width, `18 AP` add pc / bundle, `0d DT` date, `07 SZ` size, `09 BD` bundle, `06 PN` piece name, `05 PD` piece description, `12 PC` piece category, `0c SP` split piece, `11 PE` pen number, `08 LR` left / right, `0e LBA` and `19 LBB` line type A / B, `1d PA` placed area, `1e PP` piece perimeter, `1f NP` number of pieces, `22 MM` order model message, `23 CN`, `24 BG`, `25 FC`, `10 LT1` / `1b LT2` line type n, `0f SY` symbol, `0a` a constant text. Types with a length have `code 01 value` (`PN1-50` = `06 01 32`, `BG1-10` = `24 01 0a`); a constant is `0a <u16 length> text`. Read off one row that holds every type in list order (30 tokens), 0x1a appears in the `LABELS` / `LABELI` rows of real markers [?], 0x20 / 0x21 are unassigned.
* **A Symbol row keeps its parameters in the row's flags:** `SY0105` in a row made `flags = 0x0501` (`01 05`); the system row `LABELD` has flags `4a 19` (or `45 19`) and the one token `0f`.
* **What a marker copies:** not the whole table but the rows it USES - `DEFAULT`, `MARKER`, the `LABEL...` rows of its plot, and **one `-PDSTEXT-` row per piece** whose single token is a constant = the piece's PDS annotation text (its name on the corpus pieces: 49 of 50 fixture markers carry them). With `ZZANN-1` (rows `TEST1` / `TEST2` / `TEST3` too) the marker holds only DEFAULT (empty in that table), MARKER (`MSQ,/,AP,/,WI,L,U,PS`) and the three piece rows.
* **Verified:** the rows close on the byte for **225 of 225** markers of this machine (224 compact, 1 padded) and for the 50 fixture markers; `mk['annotation']` = `dict(ok, rows, end, padded)`; the byte map marks the span identified. Nothing here is needed by a nester.
